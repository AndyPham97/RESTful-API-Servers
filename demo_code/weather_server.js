...
function sendResponse(weatherData, res){
  ...
  if(weatherData){
    ...
    page += '<h1>Weather Info for ' + obj.name + '</h1><p>' + weatherData +'</p>'
}
  page += '</body></html>'
  res.end(page);
}
...
http.createServer(function (req, res) {
  ...
  } else if (req.method == "GET" && requestURL === "/?city=Ottawa") {
    var index = requestURL.indexOf("?city=");
    if (index > 0) {
      var name = requestURL.substring(index + "?city=".length, requestURL.length);
      getWeather(name, res);
    }
  }
  else{
